...
const RoomDetailScreen = ({ route, navigation }) => {
  ...
  const handleZalo = async () => {
    const phone = room?.contactPhone?.replace(/\D/g, '');
    if (!phone) {
      Alert.alert('Thông báo', 'Không có số điện thoại Zalo');
      return;
    }

    // Prefer deep link to profile if UID is available, otherwise phone-based link.
    const uid = room?.ownerId?.zaloUid || room?.zaloUid;
    const url = uid ? `zalo://profile?uid=${uid}` : `https://zalo.me/${phone}`;
    ...
    try {
      ...
      if (!supported) {
        Alert.alert(
          'Không thể mở Zalo',
          'Vui lòng cài đặt Zalo hoặc liên hệ qua gọi/SMS. '
        );
        return;
      }
      await Linking.openURL(url);
    } catch (error) {
      console.error('Zalo open error:', error);
      Alert.alert('Lỗi', 'Không thể mở Zalo. Vui lòng thử lại sau.');
    }
  };
